...
const privatePools = locations.filter(location => location.Type === 'Private');
const publicPools = locations.filter(location => location.Type === 'Public');

const privateGeoData = geoJSON.parse(privatePools, { Point: ['Latitude', 'Longitude'] });
const publicGeoData = geoJSON.parse(publicPools, { Point: ['Latitude', 'Longitude'] });
...
map.on('load', () => drawMap(map, privateGeoData, publicGeoData)); // go get the data
